import { chromium, firefox, webkit, Browser, Page, BrowserContext, Dialog, Download, FileChooser, devices, Request, Response, Route } from 'playwright';
...
export class PlaywrightBrowserSkill {
  ...
  private requestLogs: Array<{ url: string; method: string; timestamp: number }> = [];
  private responseLogs: Array<{ url: string; status: number; timestamp: number }> = [];
  ...
  async switchPage(index: number) {
    if (!this.pages[index]) throw new Error(`页面索引 ${index} 不存在`);
  ...
  async closePage(index?: number) {
    ...
    if (!pageToClose) throw new Error('页面不存在');
  ...
  async click(selector: string, options?: { timeout?: number; button?: 'left' | 'right' | 'middle'; clickCount?: number }) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async drag(sourceSelector: string, targetSelector: string) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async waitForSelector(selector: string, options?: { timeout?: number; state?: 'attached' | 'detached' | 'visible' | 'hidden' }) {
    if (!this.page) throw new Error('浏览器未启动');
    // 使用类型断言确保类型兼容
    const waitOptions = options as any;
  ...
  async screenshot(options?: { path?: string; fullPage?: boolean; type?: 'png' | 'jpeg'; quality?: number }) {
    if (!this.page) throw new Error('浏览器未启动');
    const screenshot = await this.page.screenshot(options);
    return { success: true, screenshot: screenshot.toString('base64') };
  }

  async screenshotElement(selector: string, options?: { path?: string }) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async pdf(options?: { path?: string; format?: string; printBackground?: boolean }) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async setCookies(cookies: Array<{ name: string; value: string; domain?: string; path?: string; expires?: number; httpOnly?: boolean; secure?: boolean; sameSite?: 'Strict' | 'Lax' | 'None' }>) {
    if (!this.context) throw new Error('浏览器未启动');
  ...
  async route(url: string | RegExp, handler: 'abort' | 'continue' | 'fulfill') {
    if (!this.page) throw new Error('浏览器未启动');
    await this.page.route(url, async (route) => {
      if (handler === 'abort') await route.abort();
      else if (handler === 'continue') await route.continue();
      else await route.fulfill({ status: 200, body: 'Mocked' });
    });
  ...
  async emulateMedia(options: { colorScheme?: 'light' | 'dark' | 'no-preference'; media?: 'screen' | 'print' }) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  private setupPageEventListeners(page: Page) {
    // Console 日志
    page.on('console', (msg) => {
  ...
  async getConsoleLogs(limit?: number) {
  ...
  async getResponseLogs(limit?: number) {
  ...
  async waitForEvent(eventName: string, timeout?: number) {
    if (!this.page) throw new Error('浏览器未启动');
    await this.page.waitForEvent(eventName as any, { timeout });
    return { success: true, event: eventName };
  }

  // ==================== 高级网络控制 ====================
  
  async blockRequests(patterns: string[]) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async mockResponse(urlPattern: string | RegExp, response: { status?: number; body?: string; contentType?: string }) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async unroute(urlPattern?: string | RegExp) {
    if (!this.page) throw new Error('浏览器未启动');
    if (urlPattern) {
  ...
  async keyboardInsertText(text: string) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async mouseDown(options?: { button?: 'left' | 'right' | 'middle'; clickCount?: number }) {
    if (!this.page) throw new Error('浏览器未启动');
    await this.page.mouse.down(options);
    return { success: true };
  }

  async mouseUp(options?: { button?: 'left' | 'right' | 'middle'; clickCount?: number }) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async mouseClick(x: number, y: number, options?: { button?: 'left' | 'right' | 'middle'; clickCount?: number; delay?: number }) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async getAccessibilitySnapshot(selector?: string) {
    if (!this.page) throw new Error('浏览器未启动');
    // 使用类型断言解决accessibility属性问题
    const pageWithAccessibility = this.page as any;
  ...
  async getByText(text: string | RegExp, options?: { exact?: boolean }) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async getByLabel(text: string | RegExp, options?: { exact?: boolean }) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async getByPlaceholder(text: string | RegExp, options?: { exact?: boolean }) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async getByAltText(text: string | RegExp, options?: { exact?: boolean }) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async getByTitle(text: string | RegExp, options?: { exact?: boolean }) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async waitForFunction(fn: string, arg?: any, options?: { timeout?: number; polling?: number }) {
    if (!this.page) throw new Error('浏览器未启动');
  ...
  async restoreStorageState(state: any) {
  ...
  async isConnected() {
    if (!this.browser) return { success: true, connected: false };
